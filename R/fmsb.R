...
radarchart <- function(df, axistype=0, seg=4, pty=16, pty.cex=1, pcol=1:8, plty=1:6, plwd=1,
                       pdensity=NULL, pangle=45, pfcol=NA, cglty=3, cglwd=1,
                       cglcol="navy", axislabcol="blue", maxmin=TRUE,
                       na.itp=TRUE, centerzero=FALSE, vlabels=NULL, vlcex=NULL,
                       caxislabels=NULL, calcex=NULL, paxislabels=NULL, palcex=NULL,
                       title="", title.cex=0.8, title.line=0,
                       legend.x=NULL, legend.y=NULL, legend.pos="topright",
                       legend.cex=0.64, legend.ncol=1, legend.y.intersp = 0, legend.xpd = TRUE, ...) {
  ...
  plot(c(-1.2, 1.2), c(-1.2, 1.2), type="n", frame.plot=FALSE, axes=FALSE,
       xlab="", ylab="", asp=1, ...) # define x-y coordinates without any plot
  ...
  if (is.null(vlcex)) text(xx*1.18, yy*1.18, VLABELS, col="black") else
    text(xx*1.18, yy*1.18, VLABELS, cex=vlcex, col="black")
  series <- length(df[[1]])
  SX <- series-2
  ...
  for (i in 3:series) {
    ...
    if (sum(!is.na(df[i,]))<3) { cat(sprintf("[DATA NOT ENOUGH] at %d\n%g\n",i,df[i,])) # for too many NA's (1.2.2012)
      ...
      points(xx*scale, yy*scale, pch=ptys[i-2], col=pcols[i-2], cex=pty.cex)
    }
  }
  title(main=title, cex.main=title.cex, line=title.line, outer=F)
  if (is.null(legend.x) || is.null(legend.y)) {
    legend(legend.pos, legend=rownames(df[-c(1, 2),]),ncol = legend.ncol, col=pcol, lty=plty, lwd=plwd, bty = "n", pch=ptys, cex=legend.cex, pt.cex=pty.cex, merge=TRUE, text.col="black", y.intersp = legend.y.intersp, xpd = legend.xpd)
  } else {
    legend(legend.x, legend.y, legend=rownames(df[-c(1, 2),]),ncol = legend.ncol, col=pcol, lty=plty, bty = "n", lwd=plwd, pch=ptys, cex=legend.cex, pt.cex=pty.cex, merge=TRUE, text.col="black", y.intersp = legend.y.intersp, xpd = legend.xpd)
  }
}
